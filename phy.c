...
u32 rtl92d_phy_query_bb_reg(struct ieee80211_hw *hw, u32 regaddr, u32 bitmask)
{
	...
	if (rtlhal->during_mac1init_radioa || rtlhal->during_mac0init_radiob) {
		...
		originalvalue = rtl92du_read_dword_dbi(hw, (u16)regaddr,
			dbi_direct);
	} else {
			   ...
			   u32 regaddr, u32 bitmask, u32 data)
{
	...
	if (bitmask != MASKDWORD) {
		if (rtlhal->during_mac1init_radioa ||
			...
			originalvalue = rtl92du_read_dword_dbi(hw,
					(u16) regaddr,
					dbi_direct);
		else
			originalvalue = rtl_read_dword(rtlpriv, regaddr);
	...
	if (rtlhal->during_mac1init_radioa || rtlhal->during_mac0init_radiob)
		rtl92du_write_dword_dbi(hw, (u16) regaddr, data, dbi_direct);
	else
...
void rtl92d_update_bbrf_configuration(struct ieee80211_hw *hw)
{
	...
	if (rtlphy->rf_type == RF_1T1R) {
		...
		if (rtlhal->interfaceindex == 0) {
			...
			/* 0x888 */
			rtl92du_write_dword_dbi(hw, RFPGA0_ADDALLOCKEN,
						rtl92du_read_dword_dbi(hw,
						RFPGA0_ADDALLOCKEN,
						BIT(3)) | BIT(12) | BIT(13),
						BIT(3));
			rtl92d_phy_powerdown_anotherphy(hw, false);
