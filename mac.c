...
void rtl92d_read_chip_version(struct ieee80211_hw *hw)
{
	...
	enum version_8192d chip_version = VERSION_UNKNOWN;
	const char *versionid;
	u32 value32;

	value32 = rtl_read_dword(rtlpriv, REG_SYS_CFG);
	if (value32 & TRP_VAUX_EN) {
		chip_version = (value32 & TYPE_ID) ? VERSION_TEST_CHIP_92D :
			       VERSION_TEST_CHIP_88C;
	} else {
		/* Normal mass production chip. */
		chip_version = NORMAL_CHIP;
		chip_version |= ((value32 & TYPE_ID) ? CHIP_92D : 0);
		...
		if (IS_92D_SERIAL(chip_version)) {
			value32 = rtl_read_dword(rtlpriv, REG_HPON_FSM);
			chip_version |= ((CHIP_BONDING_IDENTIFIER(value32) ==
				 CHIP_BONDING_92D_1T2R) ? CHIP_92D_1T2R : 0);
	...
	rtlhal->version  = (enum version_8192d)chip_version;
	pr_info("Chip version 0x%x\n", chip_version);
	switch (rtlhal->version) {
	case VERSION_NORMAL_TSMC_CHIP_92D_1T2R:
		versionid = "NORMAL_B_CHIP_92D";
		break;
	case VERSION_NORMAL_TSMC_CHIP_92D:
		versionid = "NORMAL_TSMC_CHIP_92D";
		break;
	case VERSION_NORMAL_TSMC_CHIP_88C:
		versionid = "NORMAL_TSMC_CHIP_88C";
		break;
	case VERSION_NORMAL_UMC_CHIP_92D_1T2R_A_CUT:
		versionid = "NORMAL_UMC_CHIP_i92D_1T2R_A_CUT";
		break;
	case VERSION_NORMAL_UMC_CHIP_92D_A_CUT:
		versionid = "NORMAL_UMC_CHIP_92D_A_CUT";
		break;
	case VERSION_NORMAL_UMC_CHIP_88C_A_CUT:
		versionid = "NORMAL_UMC_CHIP_88C_A_CUT";
		break;
	case VERSION_NORMAL_UMC_CHIP_92D_1T2R_B_CUT:
		versionid = "NORMAL_UMC_CHIP_92D_1T2R_B_CUT";
		break;
	case VERSION_NORMAL_UMC_CHIP_92D_B_CUT:
		versionid = "NORMAL_UMC_CHIP_92D_B_CUT";
		break;
	...
	case VERSION_NORMA_UMC_CHIP_8723_1T1R_B_CUT:
		...
		break;
	case VERSION_TEST_CHIP_92D:
		versionid = "TEST_CHIP_92D";
		break;
...
/**
 * rtl92d_init_LLT_table - Init LLT table
 * @io: io callback
 * @boundary:
 *
 * Realtek hardware access function.
...
bool rtl92d_init_llt_table(struct ieee80211_hw *hw, u32 boundary)
{
	...
	/* end of list */
	rst = rtl92d_llt_write(hw, (boundary - 1), 0xFF);
	if (true != rst) {
		pr_err("===> %s #2 fail\n", __func__);
		return rst;
	}
	...
	rst = rtl92d_llt_write(hw, LLT_LAST_ENTRY_OF_TX_PKT_BUFFER, boundary);
	if (true != rst) {
		pr_err("===> %s #4 fail\n", __func__);
		return rst;
	}
...
void rtl92d_enable_interrupt(struct ieee80211_hw *hw)
{
	...
	if (IS_HARDWARE_TYPE_8192DE(rtlhal)) {
...
{
	rtl92d_set_network_type(hw, NL80211_IFTYPE_UNSPECIFIED);
}
...
static void rtl92d_set_cck_sifs(struct ieee80211_hw *hw, u8 trx_sifs,
				u8 ctx_sifs)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
static void rtl92d_set_ofdm_sifs(struct ieee80211_hw *hw, u8 trx_sifs,
				 u8 ctx_sifs)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
{
	/* sequence: VO, VI, BE, BK ==> the same as 92D hardware design.
	...
	value |= (u32)txop << 16;
	/* 92D hardware register sequence is the same as queue number. */
...
void rtl92d_init_edca(struct ieee80211_hw *hw)
{
	...
	rtl92d_set_cck_sifs(hw, 0xa, 0xa);
	rtl92d_set_ofdm_sifs(hw, 0xe, 0xe);
...
static long _rtl92d_signal_scale_mapping(struct ieee80211_hw *hw,
		long currsig)
{
	long retsig;
				      ...
				      bool packet_beacon)
{
	...
	if (is_cck_rate)
		pstats->signalstrength =
		    (u8) (_rtl92d_signal_scale_mapping(hw, pwdb_all));
	else if (rf_rx_num != 0)
		pstats->signalstrength =
		    (u8) (_rtl92d_signal_scale_mapping
