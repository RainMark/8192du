...
u32 rtl92du_read_dword_dbi(struct ieee80211_hw *hw, u16 offset, u8 direct)
{
...
void rtl92du_write_dword_dbi(struct ieee80211_hw *hw,
...
static void _rtl92du_resume_tx_beacon(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u8 tmp1byte;
...
void rtl92du_get_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	struct rtl_mac *rtlpci = rtl_mac(rtl_priv(hw));
	...
	case HW_VAR_H2C_FW_JOINBSSRPT: {
		...
		if (mstatus == RT_MEDIA_CONNECT) {
			...
			rtl_write_byte(rtlpriv, REG_CR + 1,
				       (tmp_regcr | BIT(0)));
			_rtl92du_set_bcn_ctrl_reg(hw, 0, BIT(3));
			_rtl92du_set_bcn_ctrl_reg(hw, BIT(4), 0);
			...
			_rtl92du_set_bcn_ctrl_reg(hw, BIT(3), 0);
			_rtl92du_set_bcn_ctrl_reg(hw, 0, BIT(4));
			if (recover)
...
static bool _rtl92du_llt_write(struct ieee80211_hw *hw, u32 address, u32 data)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	bool status = true;
	long count = 0;
...
static bool _rtl92du_llt_table_init(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	unsigned short i;
	u8 txpktbuf_bndy;
	...
	for (i = 0; i < (txpktbuf_bndy - 1); i++) {
		status = _rtl92du_llt_write(hw, i, i + 1);
		if (true != status)
			return status;
	}

	/* end of list */
	status = _rtl92du_llt_write(hw, (txpktbuf_bndy - 1), 0xFF);
	if (true != status)
		return status;
	...
	status = _rtl92du_llt_write(hw, maxPage, txpktbuf_bndy);
	if (true != status)
		return status;
...
static void _rtl92du_gen_refresh_led_state(struct ieee80211_hw *hw)
{
	...
	if (ppsc->rfoff_reason == RF_CHANGE_BY_IPS)
		rtl92du_sw_led_on(hw, pLed0);
	else if (ppsc->rfoff_reason == RF_CHANGE_BY_INIT)
		rtl92du_sw_led_on(hw, pLed0);
	else
		rtl92du_sw_led_off(hw, pLed0);
}
...
static bool _rtl92du_init_mac(struct ieee80211_hw *hw)
{
  ...
  u8 out_ep_numbers = rtlusb->out_ep_nums;
	...
	_rtl92du_gen_refresh_led_state(hw);
...
static void _rtl92du_hw_configure(struct ieee80211_hw *hw)
{
	struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
...
static void _rtl92du_enable_aspm_back_door(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_ps_ctl *ppsc = rtl_psc(rtl_priv(hw));
...
void rtl92du_enable_hw_security_config(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u8 sec_reg_value;
...
int rtl92du_hw_init(struct ieee80211_hw *hw)
{
  ...
  _rtl92du_hw_configure(hw); /* TODO */
	...
	rtl92du_enable_hw_security_config(hw);
	...
	_rtl92du_enable_aspm_back_door(hw);
...
void rtl92du_interrupt_recognized(struct ieee80211_hw *hw,
...
static void _rtl92du_readpowervalue_fromprom(struct txpower_info *pwrinfo,
...
static void _rtl92du_read_txpower_info(struct ieee80211_hw *hw,
				       bool autoload_fail, u8 *hwinfo)
{
	...
	_rtl92du_readpowervalue_fromprom(&pwrinfo, hwinfo, autoload_fail);
	if (!autoload_fail) {
		/* bit0~2 */
...
static void _rtl92du_read_macphymode_from_prom(struct ieee80211_hw *hw,
					       u8 *content)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
...
static void _rtl92du_read_macphymode_and_bandtype(struct ieee80211_hw *hw,
						  u8 *content)
{
	_rtl92du_read_macphymode_from_prom(hw, content);
	rtl92d_phy_config_macphymode(hw);
	rtl92d_phy_config_macphymode_info(hw);
}

static void _rtl92du_efuse_update_chip_version(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
static void _rtl92du_read_adapter_info(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	...
	if (rtlefuse->epromtype == EEPROM_BOOT_EFUSE) {
		...
		_rtl92du_efuse_update_chip_version(hw);
		spin_unlock_irqrestore(&globalmutex_for_power_and_efuse, flags);
	...
	_rtl92du_read_macphymode_and_bandtype(hw, hwinfo);
	...
	_rtl92du_read_txpower_info(hw, rtlefuse->autoload_failflag, hwinfo);
...
void rtl92du_read_eeprom_info(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	...
	rtlhal->version = _rtl92du_read_chip_version(hw);
...
static void rtl92du_update_hal_rate_table(struct ieee80211_hw *hw,
...
static void rtl92du_update_hal_rate_mask(struct ieee80211_hw *hw,
...
void rtl92du_update_hal_rate_tbl(struct ieee80211_hw *hw,
		struct ieee80211_sta *sta, u8 rssi_level)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);

	if (rtlpriv->dm.useramask)
		rtl92du_update_hal_rate_mask(hw, sta, rssi_level);
	else
		rtl92du_update_hal_rate_table(hw, sta);
}

void rtl92du_update_channel_access_setting(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_mac *mac = rtl_mac(rtl_priv(hw));
	u16 sifs_timer;
...
bool rtl92du_gpio_radio_on_off_checking(struct ieee80211_hw *hw, u8 *valid)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_ps_ctl *ppsc = rtl_psc(rtl_priv(hw));
	struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
...
void rtl92du_set_key(struct ieee80211_hw *hw, u32 key_index,
...
void rtl92du_suspend(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
...
void rtl92du_resume(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
